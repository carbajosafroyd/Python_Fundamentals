...
A variable can have a short name, like `x` and `y`, or a more descriptive name, such as `age`, `carname`, or `total_volume`.
...
The best way to output multiple variables in the `print()` function is to separate them with commas. This also supports different data types.
...
Variables that are created outside of a function, as in the previous examples, are known as global variables.
